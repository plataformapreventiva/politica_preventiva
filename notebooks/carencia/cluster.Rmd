...
```{r}
data("df_mxmunicipio")
coneval_complete <- coneval_data %>% select(starts_with("ic_"), cve_muni) %>% drop_na()
...
df_mxmunicipio$value <- ordered(df_mxmunicipio$value, levels = c("1","2","4","3","5"),
                                labels =  c("Muy Bajo", "Bajo", "Medio", "Alto", "Muy Alto"))

 cols <- c("Muy Bajo"= "#1a9641", 
           "Bajo"= "#a6d96a", 
           "Medio"= "#ffffbf", 
           "Alto"= "#fdae61", 
           "Muy Alto"= "#d7191c")
...
map<-mxmunicipio_choropleth(df_mxmunicipio, num_colors = 5) +
