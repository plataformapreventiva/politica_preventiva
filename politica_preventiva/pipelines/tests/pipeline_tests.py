...
def dictionary_test(task, dict_path, table_header, dict_header,
	...
	s3_ = client('s3')
	s3 = resource('s3')
	file_name = common_key + task + '_dic.csv'
	try:
		try:
			obj = s3_.get_object(Bucket=common_bucket, Key= file_name)
			local_dictionary = pd.read_csv(obj['Body'], sep='|', index_col='id')
		except:
		...
		assert True not in dictionary['nombre'].isnull().values,\
		...
		dictionary['metadata'] = '{}'
		dictionary['actualizacion_sedesol'] = current_date
		dictionary['data_date'] = list(data_date + '-' + suffix)
