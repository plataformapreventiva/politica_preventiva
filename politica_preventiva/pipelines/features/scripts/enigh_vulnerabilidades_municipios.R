...
if(length(opt) > 1){
  ...
  query2 <- 'SELECT * FROM features.enigh_vulnerabilidades_estados'
  ...
  copy_to(con, municipal,
          dbplyr::in_schema("features","enigh_vulnerabilidades_municipios"),
          temporary = FALSE, overwrite = TRUE)
  dbDisconnect(con)

  print('Features written to: features.enigh_vulnerabilidades_municipios')
}
