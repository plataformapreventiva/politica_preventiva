...
create or replace function colpivot(
    ...
    begin
        ...
        execute ('create temp table ' || quote_ident(out_table) || ' as ' || query);
...
create or replace function make_cols_not_string(tname text, sname text) returns void as 
$$
declare
	cname text;
begin
	...
	LOOP
		if cname <> lower(cname) then
			execute format('alter table %s.%I rename %I to %I', 
					sname, tname, cname, replace(cname, '''', ''));
		end if;
	end LOOP;
end 
$$ language plpgsql;
