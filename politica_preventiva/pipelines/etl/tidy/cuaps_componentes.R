...
if(length(opt) > 1){
  ...
  filtered_tidy <- map_df(1:nrow(names_df), function(x) tidy_count(data = cuaps_componentes,
  ...
  unfiltered_tidy <- map_df(1:nrow(names_df), function(x) tidy_count(data = cuaps_componentes,
                                                     count_var = names_df$varname[x],
                                                     plotname = names_df$plotname[x],
                                                     subset = names_df$subset[[x]]))

  cuaps_componentes_tidy <- bind_rows(filtered_tidy, unfiltered_tidy) %>%
