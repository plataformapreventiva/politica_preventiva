...
municipios <- municipios %>% dplyr::mutate_each(funs(as.numeric(.)),matches("^[s|p_|e|u][0-9]+", ignore.case=FALSE))
...
## Explorador Estados
##################
estados = as_tibble(dbGetQuery(con, "select * from clean.explorador_estados WHERE cve_ent!='33' AND cve_ent IS NOT NULL;")) 
...
estados <- estados %>% dplyr::mutate_each(funs(as.numeric(.)),matches("^[s|p_|n_|e|u][0-9]+", ignore.case=FALSE))
